    ...
    for (uint32_t i = 0; i < nitems(modbusBridgeTCP.client_tcp); i++)
    {
      ...
      if (client)
      {
        ...
        else if (modbusBridge.buffer[1] <= 4)
        {
          ...
          print_hex_dual(header, 9, modbusBridge.buffer + 3, modbusBridge.byteCount);
        }
        else
        {
          ...
          print_hex_dual(header, 8, modbusBridge.buffer + 2, 4);
          nrOfBytes += 4;
        }
//dw        client.flush();
        AddLog(LOG_LEVEL_DEBUG, PSTR("[1] MBS: MBRTCP from Modbus TransactionId:%d, deviceAddress:%d, writing %d bytes to client"), (static_cast<uint16_t>(header[0]) << 8) + header[1], modbusBridge.buffer[0], nrOfBytes);
      }
    }
#endif
...
void ModbusTCPHandle(void)
{
  ...
  do
  {
    ...
    for (uint32_t i = 0; i < nitems(modbusBridgeTCP.client_tcp); i++)
    {
      ...
      if (buf_len >= 12)
      {
        const uint8_t mbdeviceaddress = static_cast<uint8_t>(modbusBridgeTCP.tcp_buf[6]);
        const ModbusBridgeFunctionCode mbfunctioncode  = static_cast<ModbusBridgeFunctionCode>(modbusBridgeTCP.tcp_buf[7]);
        const uint16_t mbstartaddress  = (static_cast<uint16_t>(modbusBridgeTCP.tcp_buf[8]) << 8) + modbusBridgeTCP.tcp_buf[9];
        ...
        if (mbfunctioncode <= ModbusBridgeFunctionCode::mb_readInputStatus) // Multiple Coils, Inputs
        {
        ...
        else if (mbfunctioncode <= ModbusBridgeFunctionCode::mb_readInputRegisters) // Multiple Holding or input registers
        {
        ...
        else // Write coil(s) or register(s)
        {
          ...
          const uint8_t dataStartByte = mbfunctioncode <= ModbusBridgeFunctionCode::mb_writeSingleRegister ? 10 : 13;
          ...
          if ((mbfunctioncode == ModbusBridgeFunctionCode::mb_writeMultipleCoils) || (modbusBridge.functionCode == ModbusBridgeFunctionCode::mb_writeMultipleRegisters))
            count = (static_cast<uint16_t>(modbusBridgeTCP.tcp_buf[10]) << 8) + modbusBridgeTCP.tcp_buf[11];
          else
            count = 1;
        ...
        AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("[2] MBS: MBRTCP to Modbus TransactionId:%d, deviceAddress:%d, functionCode:%d, startAddress:%d, count:%d, recvCount:%d, recvBytes:%d"),
               modbusBridgeTCP.tcp_transaction_id, mbdeviceaddress, mbfunctioncode, modbusBridge.startAddress, count, modbusBridge.dataCount, modbusBridge.byteCount);

        modbusBridgeModbus->Send(mbdeviceaddress, static_cast<uint8_t>(mbfunctioncode), mbstartaddress, count, writeData);

        if (modbusBridgeTCP.output_mqtt)
        {
          modbusBridge.deviceAddress = mbdeviceaddress;
          modbusBridge.functionCode = mbfunctioncode;
